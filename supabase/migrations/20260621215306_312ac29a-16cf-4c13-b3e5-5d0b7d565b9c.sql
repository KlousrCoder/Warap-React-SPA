...
DROP FUNCTION IF EXISTS public.create_project(text, text, numeric, text);
DROP FUNCTION IF EXISTS public.get_open_projects();

CREATE OR REPLACE FUNCTION public.create_project(_title text, _description text, _budget numeric, _city text, _category text DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_open_projects()
 RETURNS TABLE(id uuid, title text, description text, budget numeric, city text, category text, created_at timestamp with time zone, client_id uuid, client_name text, client_avatar text, application_count integer, max_applications integer, my_rank integer, has_applied boolean)
 LANGUAGE sql
...
AS $function$
  ...
  SELECT p.id, p.title, p.description, p.budget, p.city, p.category, p.created_at,
